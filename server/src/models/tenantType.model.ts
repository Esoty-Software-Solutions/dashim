...
async function findMany(userId: string, params?: Prisma.TenantTypeFindManyArgs) {
  return await enhancedPrisma(userId).tenantType.findMany(params);
}

async function findUnique(userId: string, params: Prisma.TenantTypeFindUniqueArgs) {
  return await enhancedPrisma(userId).tenantType.findUnique(params);
}

async function findFirst(userId: string, params: Prisma.TenantTypeFindFirstArgs) {
  return await enhancedPrisma(userId).tenantType.findFirst(params);
}
...
async function count(userId: string, params?: Prisma.TenantTypeCountArgs) {
  return await enhancedPrisma(userId).tenantType.count(params);
}

async function aggregate(userId: string, params: Prisma.TenantTypeAggregateArgs) {
  return await enhancedPrisma(userId).tenantType.aggregate(params);
}
